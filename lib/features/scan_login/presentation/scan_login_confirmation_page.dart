...
class _ScanLoginConfirmationPageState
  ...
  void _startCountdown(ScanLoginRequest request) {
    ...
    _expiresAt = request.expiredTime ??
        DateTime.now()
            .add(ref.read(appEnvironmentProvider).scanLoginFallbackExpires);
    _tickCountdown();
    if (!_expired) {
  ...
  void _tickCountdown() {
    ...
    final seconds = difference.inMilliseconds <= 0
        ? 0
  ...
  Future<void> _leaveAfterExpiry() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('登录二维码已过期，请重新扫码。')),
    );
    context.go('/');
  }
  ...
  Future<void> _submit({required bool approved}) async {
    ...
    if (!success) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(approved ? '已同意本次登录。' : '已拒绝本次登录。')),
    );
    context.go('/');
  }
            ...
            child: Padding(
              ...
              child: controller.loading
                  ...
                  : controller.error != null && request == null
                      ? _ErrorState(
                          message: controller.expired
                              ? '登录二维码已过期，请重新扫码。'
                              : '加载登录请求失败，请重试。',
                          actionLabel: controller.expired ? '返回重新扫码' : '重试',
                          onRetry:
                              controller.expired ? _leaveAfterExpiry : _load,
                        )
                      : request == null
